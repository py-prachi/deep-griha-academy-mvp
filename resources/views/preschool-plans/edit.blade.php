                ...
                <div class="col ps-4 pe-4">
                    ...
                    <nav aria-label="breadcrumb" class="mb-2">
                        <ol class="breadcrumb small">
                            <li class="breadcrumb-item"><a href="{{ route('preschool-plans.index', ['class_id' => $plan->class_id, 'section_id' => $plan->section_id]) }}">Pre-School Plans</a></li>
                            <li class="breadcrumb-item active">Edit Daily Plan</li>
                        </ol>
                    </nav>

                    <h4 class="mb-1"><i class="bi bi-stars me-1"></i> Edit Daily Plan</h4>
                    <p class="text-muted small mb-3">
                        {{ $plan->schoolClass->class_name }} {{ $plan->section->section_name }}
                        &mdash; {{ $plan->plan_date->format('d M Y') }}
                    </p>
                    ...
                    <div class="card shadow-sm" style="max-width:900px;">
                        <div class="card-body">
                            <form method="POST" action="{{ route('preschool-plans.update', $plan->id) }}">
                                @csrf @method('PUT')
                                ...
                                <div class="mb-4">
                                    ...
                                    <input type="date" name="plan_date" class="form-control @error('plan_date') is-invalid @enderror"
                                           value="{{ old('plan_date', $plan->plan_date->format('Y-m-d')) }}" required style="max-width:200px;">
                                ...
                                <div id="slots-container">
                                    @php
                                        $editSlots = old('slots');
                                        if (!$editSlots) {
                                            $editSlots = $plan->slots->map(function($s) {
                                                return [
                                                    'activity_name' => $s->activity_name,
                                                    'material'      => $s->material,
                                                    'objective'     => $s->objective,
                                                    'actual_teach'  => $s->actual_teach,
                                                    'assessment'    => $s->assessment,
                                                ];
                                            })->toArray();
                                        }
                                        if (empty($editSlots)) {
                                            $editSlots = [['activity_name'=>'','material'=>'','objective'=>'','actual_teach'=>'','assessment'=>'']];
                                        }
                                    @endphp

                                    @foreach($editSlots as $i => $slot)
                                ...
                                <div class="d-flex gap-2">
                                    ...
                                    <a href="{{ route('preschool-plans.index', ['class_id' => $plan->class_id, 'section_id' => $plan->section_id]) }}" class="btn btn-outline-secondary">Cancel</a>
                                </div>
                            </form>
                        </div>
                    </div>
