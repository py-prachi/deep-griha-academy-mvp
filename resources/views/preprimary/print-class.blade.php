...
<div class="print-toolbar no-print">
    ...
    <a href="{{ route('preprimary.entry', ['class_id' => $class_id, 'section_id' => $section_id]) }}"
        style="text-decoration:none;color:#555;font-size:12px;margin-left:12px;">
        &larr; Back to Entry
    </a>
    <span style="font-size:12px;color:#888;margin-left:16px;">
        {{ $promotions->count() }} students &mdash; {{ $schoolClass->class_name ?? '' }} {{ $section->section_name ?? '' }}
    </span>
</div>
...
@php
    ...
    $grades     = isset($allGrades[$student_id]) ? $allGrades[$student_id] : [];
    $narratives = isset($allNarratives[$student_id]) ? $allNarratives[$student_id] : [];
@endphp
